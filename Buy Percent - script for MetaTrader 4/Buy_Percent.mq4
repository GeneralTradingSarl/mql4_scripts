...
double LotMarginForex (string symbol)
  {
   ...
   double rate_сurrency=1; if (base!=сurrency) rate_сurrency=MarketInfo(base+сurrency,MODE_ASK);
...
double LotMarginCFD (string symbol)
  {
   string сurrency=AccountCurrency();
   double rate_сurrency=1; if (сurrency!="USD") rate_сurrency=MarketInfo(сurrency+"USD",MODE_ASK);
