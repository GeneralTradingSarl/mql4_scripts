...
//extern bool		Emulated	=true;		//	флаг разрешения реагирования на моделируемые тики
...
//----
	InitTicket(StringSubstr(IndicatorName,2,StringLen(IndicatorName)-2));					// -->
...
int start()
{
	...
	EmptyBufferam();
//+-------------------------- Begin Cycle ---------------------------------------------------+\\
	ReadValueTicket(Shift,LRCalc);											// -->
	
	for (Ix=0;Ix<CalcCount;Ix++) Rates[Ix]=Close[Ix+Shift];			// Чтение данных с установленым смещением

	LineRegressOnArray(Rates,TimeBuf1,DeGree,LRCalc);

	// Вариан №1 - вывод результата
	ArrayCopy(FxView1,TimeBuf1,Shift,0,LRCalc);							// Отображение результатов расччета с установленным смещением
	// Вариан №2 - вывод результата
	for (Ix=LRCalc;Ix>=0;Ix--)
	{
		FxView1[Ix+Shift]=TimeBuf1[Ix];										// Отображение результатов расччета с установленным смещением
	}
